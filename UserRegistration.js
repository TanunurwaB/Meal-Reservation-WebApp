...
const UserRegistration = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const userData = { username, email, password, phone, category }; // Include category
    ...
    try {
        ...
        if (response.status === 201) {
            ...
            // setUser Email(email); // Uncomment if you want to set user email in context
            navigate('/login'); // Navigate to the dashboard after successful registration
        } else {
